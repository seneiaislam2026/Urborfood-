...
export default function StaffManagement() {
  ...
  useEffect(() => {
    ...
    const savedAttendance = localStorage.getItem('urbor_staff_attendance');
    if (savedAttendance) setAttendance(JSON.parse(savedAttendance));
  }, []);

  const saveStaff = (newStaffList: StaffMember[]) => {
    setStaffList(newStaffList);
    localStorage.setItem('urbor_staff_list', JSON.stringify(newStaffList));
  };

  const saveAttendance = (newAttendance: AttendanceRecord[]) => {
    setAttendance(newAttendance);
    localStorage.setItem('urbor_staff_attendance', JSON.stringify(newAttendance));
  };
