  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
    ...
    setFormData(prev => {
      ...
      if (name === 'phone' && value.length >= 11) {
        ...
        if (existingOrder) {
          if (!prev.name) newData.name = existingOrder.customerName;
          if (!prev.address) newData.address = existingOrder.address;
        }
      }
      return newData;
    });
            ...
            <form id="checkout-form" onSubmit={handleCheckoutSubmit} className="flex flex-col gap-4 animate-in fade-in zoom-in duration-300">
              <div className="bg-primary/5 p-4 rounded-xl border border-primary/10 mb-2">
                <div className="flex justify-between items-center text-sm font-bold text-slate-800">
                  <span>মোট বিল:</span>
                  <span>&nbsp;৳{toBanglaNumber(cartTotal)}</span>
                </div>
              </div>
              ...
              return (
                <div key={item.id} className="flex gap-4 bg-white p-3 rounded-xl border border-gray-100 shadow-sm relative pr-10 group">
                  <div className="w-20 h-20 bg-slate-50 rounded-lg overflow-hidden flex-shrink-0">
                    <ImageLoader src={item.image} alt={item.name} className="w-full h-full object-cover" />
                  </div>
                  <div className="flex flex-col flex-1 py-1">
                    <h3 className="text-sm font-bold text-slate-800 leading-tight mb-1">{item.name}</h3>
                    <div className="text-primary font-bold text-sm mb-2">&nbsp;৳{toBanglaNumber(price)}</div>
          ...
          <div className="p-6 border-t border-gray-100 bg-gray-50">
            {!isCheckingOut && (
              <div className="flex items-center justify-between mb-4">
                ...
                <span className="text-lg font-extrabold text-slate-800">&nbsp;৳{toBanglaNumber(cartTotal)}</span>
              </div>
            )}
            {isCheckingOut ? (
              <button 
