...
export default function TopBannerNotification({ isAdminView = false }: { isAdminView?: boolean }) {
  ...
  useEffect(() => {
    ...
    const handleBeforeInstallPrompt = (e: Event) => {
      ...
      const isDismissed = safeSessionGetItem('mega_pwa_banner_dismissed') === 'true';
      if (!isDismissed) {
    ...
    const handleAppInstalled = () => {
      ...
      console.log('Urbor Food PWA installed successfully by user!');
    };
  ...
  const handleDismissPrompt = () => {
    setShowInstallPrompt(false);
    safeSessionSetItem('mega_pwa_banner_dismissed', 'true');
  };
        ...
        <div className="w-full max-w-lg bg-black text-white shadow-2xl rounded-2xl p-4 border border-zinc-800 flex items-start gap-4 pointer-events-auto transform translate-y-0 animate-in slide-in-from-top-12 duration-500 ease-out">
          ...
          <div className="flex-1 flex flex-col">
            <div className="flex items-center gap-1.5">
              <span className="bg-zinc-800 text-zinc-300 text-[9px] font-black px-1.5 py-0.5 rounded leading-tight uppercase tracking-wider">PWA APP</span>
              <h4 className="text-sm font-extrabold text-white">উর্বর ফুড অ্যাপ ইনস্টল করুন!</h4>
            </div>
