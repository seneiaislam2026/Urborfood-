...
content = content.replace('export default function OrderTrackingModal()', 'export default function OrderTrackingPage()');

// Remove isOrderTrackingOpen
content = content.replace('const { isOrderTrackingOpen, setIsOrderTrackingOpen } = useUI();', '');
content = content.replace('if (!isOrderTrackingOpen) return null;', '');

// Replace outer divs to be a full page layout
const oldReturn = `  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-3 sm:p-4 md:p-6 lg:p-10">
      {/* Overlay */}
      <div 
...
// Remove close button
const closeButtonRegex = /<button\s+onClick=\{\(\) => setIsOrderTrackingOpen\(false\)\}[\s\S]*?<\/button>/;
